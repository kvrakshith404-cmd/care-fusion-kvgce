    ...
    <div className="min-h-screen gradient-bg pb-24">
      <div className="gradient-header px-5 pt-11 pb-7 rounded-b-[2.5rem] relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} className="relative">
          <h1 className="text-2xl font-extrabold text-primary-foreground tracking-tight">Smart Scanner</h1>
          <p className="text-xs text-primary-foreground/60 font-medium">AI-powered medicine & prescription analysis</p>
        </motion.div>
      </div>
      ...
      <div className="px-5 mt-6 space-y-4">
          ...
          {(["medicine", "prescription"] as const).map((type) => (
            <button
              ...
              className={`flex-1 py-3 rounded-2xl text-sm font-semibold transition-all ${
                scanType === type
                  ? "gradient-primary text-primary-foreground shadow-md glow-sm"
          ...
          {selectedImage ? (
            ...
            <div className="w-full aspect-[4/3] rounded-2xl border-2 border-dashed border-primary/20 flex flex-col items-center justify-center bg-secondary/20 relative overflow-hidden">
              ...
              <p className="text-sm text-muted-foreground text-center px-4 font-medium">
                {scanType === "medicine" ? "Scan a medicine label or pill" : "Scan your prescription document"}
              </p>
              <motion.div
                animate={{ y: ["-100%", "200%"] }}
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { icon: Pill, title: "Medicine ID", desc: "Identify any pill or tablet", color: "from-emerald-500 to-teal-400" },
            { icon: FileText, title: "Prescription OCR", desc: "Extract text from Rx", color: "from-violet-500 to-purple-400" },
          ].map((opt, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
